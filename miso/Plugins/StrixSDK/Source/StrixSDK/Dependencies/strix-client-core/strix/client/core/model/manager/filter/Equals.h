...
public:
	STRIX_CLASS(Equals, Condition)
	(
		STRIX_CLASS_ID("jp.co.soft_gear.strix.server.core.model.manager.filter.Equals")
	...
	Equals(FieldPtr left, ValueTypePtr right) : m_left(std::move(left)), m_right(std::move(right)) {}
	...
	int Compare(const Object &rhs) const override
	{
		...
		const Equals & other = static_cast<const Equals &>(rhs);
