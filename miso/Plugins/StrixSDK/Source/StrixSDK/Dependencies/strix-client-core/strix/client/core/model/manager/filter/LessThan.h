...
public:
	STRIX_CLASS(LessThan, Condition)
	(
		STRIX_CLASS_ID("jp.co.soft_gear.strix.server.core.model.manager.filter.LessThan")
	...
	LessThan(FieldPtr left, ValueTypePtr right) : m_left(std::move(left)), m_right(std::move(right)) {}
	...
	int Compare(const Object &rhs) const override
	{
		...
		const LessThan & other = static_cast<const LessThan &>(rhs);
