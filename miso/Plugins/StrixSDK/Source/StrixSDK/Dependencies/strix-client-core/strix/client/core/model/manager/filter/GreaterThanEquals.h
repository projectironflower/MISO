...
class GreaterThanEquals : public Condition {
public:
	STRIX_CLASS(GreaterThanEquals, Condition)
	(
		STRIX_CLASS_ID("jp.co.soft_gear.strix.server.core.model.manager.filter.GreaterThanEquals")
	...
	GreaterThanEquals(FieldPtr left, ValueTypePtr right) : m_left(std::move(left)), m_right(std::move(right)) {}

	virtual ~GreaterThanEquals() {}
	...
	int Compare(const Object &rhs) const override
	{
		...
		const GreaterThanEquals & other = static_cast<const GreaterThanEquals &>(rhs);
