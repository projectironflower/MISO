...
public:
	STRIX_CLASS(GreaterThan, Condition)
	(
		STRIX_CLASS_ID("jp.co.soft_gear.strix.server.core.model.manager.filter.GreaterThan")
	...
	GreaterThan(FieldPtr left, ValueTypePtr right) : m_left(std::move(left)), m_right(std::move(right)) {}
	...
	int Compare(const Object &rhs) const override
	{
		...
		const GreaterThan & other = static_cast<const GreaterThan &>(rhs);
